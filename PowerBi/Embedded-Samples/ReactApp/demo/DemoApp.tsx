...
function DemoApp (): JSX.Element {
	...
	// API end-point url to get embed config for a sample report (PULL JSON OBJECT FROM EXTERNAL SOURCE)
	// const sampleReportUrl = 'https://playgroundbe-bck-1.azurewebsites.net/Reports/SampleReport';

	// MANUALLY DEFINE JSON OBJECT FOR TESTING PURPOSES
	...
	const mockSignIn = async () => {
		...
		// IF MANUALLY SETTING JSON COMMENT AWAIT
		//const reportConfigResponse = await fetch(sampleReportUrl);
		const reportConfigResponse = sampleReportUrl;
		
		// IF MANUALLY SETTING JSON COMMENT CHECK
		//if (!reportConfigResponse.ok) {
		//	console.error(`Failed to fetch config for report. Status: ${ reportConfigResponse.status } ${ reportConfigResponse.statusText }`);
		//	return;
		//}

		// IF MANUALLY SETTING JSON COMMENT JSON CONVERT
		//const reportConfig = await reportConfigResponse.json();
		const reportConfig = reportConfigResponse;
